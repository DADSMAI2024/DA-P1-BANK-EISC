...
if page ==  pages[0] :
  ...
  st.markdown("""Notre objectif est d'analyser l'ensemble des données afin d'identifier les tendances et les facteurs qui influencent la souscription d'un compte de dépôt à terme par les clients.
  Sur le plan technique, l'analyse de ce dataset représente une occasion unique de mettre en pratique nos compétences en matière d’analyse de données, de visualisation avec Plotly, de modélisation d’algorithmes de classification et de publication sur Streamlit et Github.
...
##Variables explicatives VS variable Cible
  if button4:
    ...
    - [Analyse des contacts clients durant la campagne télémarketing](#analyse-des-contacts-clients-durant-la-campagne-telemarketing)
    - [Le profil bancaire3](#le-profil-bancaire3)
    ...
    st.markdown("<h2 id='analyse-des-contacts-clients-durant-la-campagne-telemarketing'>Analyse des contacts clients durant la campagne télémarketing</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 id='Le profil bancaire3'>le-profil-bancaire3</h2>", unsafe_allow_html=True)
